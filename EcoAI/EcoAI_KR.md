...
AI 워크로드와 온체인 연산을 동시에 고려하여 **에너지 효율성**, **비용 절감**, **성능 최적화**, **탄소 배출 저감**을 달성하는 것을 목표로 한다.  
...
GPU 전력소비($P$)와 실행시간($T$)에 따라 다음과 같이 총 에너지 사용량($E$)을 추정한다:
...
이 연구는 에너지 효율적이고 투명하며 지속가능한 Web3 생태계를 가능하게 하는 **책임 있는 AI 인프라**의 미래에 기여한다.
