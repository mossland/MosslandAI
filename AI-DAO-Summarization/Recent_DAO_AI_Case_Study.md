...
### 1.1 MakerDAO(현 Sky) 사례

> **참고 (2026년 중반 기준)**: 본 절의 MakerDAO 사례는 2024년 초 시점을 기준으로 서술되었다. 이후 2024년 8월 27일 MakerDAO는 **Sky Protocol**로 리브랜딩되었으며, 거버넌스 토큰 MKR는 SKY로(1 MKR = 24,000 SKY, 2025년 5월 거버넌스 업그레이드 1단계 배포로 MKR가 거버넌스에서 은퇴하고 SKY가 유일한 거버넌스 토큰이 됨), 스테이블코인 DAI는 1:1 후속 토큰인 **USDS**로 전환되었고, SubDAO는 'Stars'(예: Spark)로 명칭이 변경되었다. 이하 서술의 MKR/DAI/SubDAO 명칭은 2024년 초 시점의 표현으로 이해하기 바란다.
...
Aave 커뮤니티는 이러한 문제 해결을 위해 L2 사용 또는 오프체인 Snapshot 투표 등 “Gasless 투표” 방안을 논의 중이며 [10], 참여 인센티브 강화 연구도 진행 중이다 [2]. (참고: 위 약 2.4% 참여율은 2024년 초 시점의 수치이며, 이후 Aave 거버넌스 참여는 크게 증가하여 2025년 12월 표결에서는 약 180만 AAVE의 투표권이 행사되며 참여 기록을 경신하기도 하였다.)
...
|------------------|------------------------------------------------------------------|--------------------------------------------------------------------------|
| **MakerDAO**<br>(현 Sky) | 탈중앙 스테이블코인 DAI 발행 및 대출 플랫폼 (MKR 토큰 거버넌스; 2024년 8월 Sky Protocol로 리브랜딩되어 MKR→SKY, DAI→USDS) | 투표 참여 저조 및 소수 고래에 의한 지배; 복잡한 안건; 창립자 영향력 과도  [1] |
| **Aave DAO**     | 탈중앙 금융(DeFi) 대출 플랫폼 (AAVE 토큰 거버넌스)                     | 낮은 투표 참여율(2024년 초 기준 약 2.4%); 가스비 부담으로 인한 소액 보유자 기피; 소수 고래 집중 [2], [9]    |
...
- **적용 사례**:
  - MakerDAO(현 Sky)는 Endgame 플랜의 일환으로 거버넌스 AI 도구(GAIT)를 도입할 계획을 제시하였다. 2026년 중반 기준 이 계획은 Sky의 로드맵(후반 단계)에 남아 있는 항목으로, 완전한 배포 여부는 공식적으로 확인되지 않았다 [1].
...
- **적용 사례**:
  - MakerDAO(현 Sky)의 GAIT 툴은 제안 요약, 데이터 검증, 신규 제안 생성 보조 역할을 수행하도록 구상되었다 [6].
...
다만, AI 도입 시 발생할 수 있는 기술적, 윤리적, 법적 리스크에 대한 신중한 관리가 병행되어야 함을 강조한다. 이러한 융합은 나아가 혁신적 비즈니스 모델과 새로운 형태의 사회 계약의 가능성을 열어줄 것으로 기대된다.
