...
flowchart LR
    ...
    subgraph Proposal_Generation
        ...
        CModule --> Aggregator
        BModule --> Aggregator
        Aggregator --> LLM["AI Proposal Generator (LLM Engine)"]
    end
...
4. **AI Generation of Proposal Draft**: The AI Proposal Generator (a current-generation LLM via the OpenAI API) is invoked with the prepared context. The system role message instructs the model on format and tone, and the user message contains the aggregated insights and the specific request (e.g., “Write a Mossland DAO proposal for the idea described.”). The model processes this and composes a draft proposal. For instance, the output could be a well-organized text: 
...
# The OpenAI() client reads the OPENAI_API_KEY environment variable by default.
client = OpenAI()  # In practice, set OPENAI_API_KEY in the environment for security
...
**Aggregating Context and Generating Proposals**: Next, we create a component that aggregates these insights and calls the OpenAI API to generate the proposal text. We encapsulate this in a `ProposalGenerator` class. This class will assemble the prompt and handle the API call to the LLM:

```python
class ProposalGenerator:
    def __init__(self, model="gpt-5.5"):  # a current OpenAI model as of 2026; gpt-4 is legacy
        self.model = model
...
This code uses the OpenAI Python SDK’s Chat Completions client method (`client.chat.completions.create`, the current interface as of the 1.x/2.x SDK; the legacy module-level `openai.ChatCompletion.create` was removed in openai-python v1.0). The structure of `messages` follows the chat format: a system role message with context, then a user role message with the actual prompt/instructions. The model will return a message as assistant role, which we extract as the proposal text. 
...
if __name__ == "__main__":
    ...
    proposal_generator = ProposalGenerator(model="gpt-5.5")  # a current OpenAI model; gpt-4 is legacy
